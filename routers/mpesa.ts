...
import {
  createInvoice,
  updateInvoiceByMpesaIDsB2c,
  updateInvoiceByMpesaIDsc2b,
} from "../utils/invoice";
...
mpesaRouter.post("/callback", async (req: Request, res: Response) => {
  try {
    ...
    if (resultCode === 0) {
      ...
      // Update the invoice
      const invoice = await updateInvoiceByMpesaIDsc2b(
        merchantRequestID,
        checkoutRequestID,
        { status: "confirmed", mpesaResponseCallback: mpesaBody }
      );
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const lipaNaMpesaOnlinePasskey = process.env
        .MPESA_CUSTOMER_PASSKEY as string;
      const lipaNaMpesaOnlineShortcode = Number(
        process.env.MPESA_CUSTOMER_SHORT_CODE
      );
  ...
  try {
    ...
    if (ResultType === 0) {
      ...
      if (OriginatorConversationID && ConversationID) {
        const invoice = await updateInvoiceByMpesaIDsB2c(
        ...
        const loan = await createLoan(invoice);
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const SecurityCredential = process.env
        .MPESA_SECURITY_CREDENTILAS as string;
